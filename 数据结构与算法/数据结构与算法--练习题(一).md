...
1. 创建数组`seen`来储存字符出现的个数，将他的每个元素设置为0。用字符当索引时要字符的索引应该为 `letters - ’a‘`,这时索引值才是从0开始的
2. 第一次遍历，如果当前的字符为`c`， 那么 `seen[c - 'a'] = seen[c - 'a'] + 1`.
3. 第二次遍历， 创建一个栈`stack`，用来进行字典序排序。
	*  如果当前字符`c`字典序大于栈顶字符，将字符`c`入栈。
	*  如果当前字符`c`字典序小于栈顶字符，有两种情况：
		*  如果栈顶字符在`seen`数组中的值大于0时，将此元素出栈。同时更新`senn`数组中的值`-1`,继续和栈顶元素进行比较
		*  如果栈顶字符在`seen`数组中的值为0，说明后面不会出现了，就不能将这个字符出栈了。
	*  将当前字符`c`入栈。
